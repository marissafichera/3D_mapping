...
CONFIG_FIN_V = {'ROOT': r'W:\regional\3d_delaware_basin\de_data\de_geodata\working_data\geosurfaces_v12_2_12082021'}
...
def copy_to_version_folder(versionrasters, extension):
    ...
    output_location = make_network_gdb_name('geosurfaces_v12_2_12082021')

    version_names = ['landsurface_SENM','alvbase','udockumbase','srtop','ldockumbase','deweylakebase','uochoanbase','lochoanbase','artesiabase',
                     'capitantop','capitanbase']
    version_names = ['landsurface_SENM', 'alvbase', 'udockumbase', 'srtop', 'ldockumbase', 'deweylakebase',
                     'uochoanbase', 'lochoanbase', 'artesiabase', 'capitanbase']
    ...
    print('MISSION COMPLETE!!!!! !!! !!!!!!!!!!!!!!!!! NOW GO RAFTING!!!!')
...
def mask_ras_with_geo_outcrop(inRasters, maskdata):
    ...
    for raster in inRasters:
        ...
        outExtractByMask.save(
            r'E:\DelawareBasin\DelawareBasin_geodata\workingdata\QCoutputs_v12_2.gdb\step07_EXBYMASK_GEO_{}_discext_v12_2'.format(
                raster))
...
def ebm_modelbound(rasters, extension, unitnames):
    ...
    for raster, unitname in zip(inRasters, unitnames):
        ...
        outExtractByMask.save(
            r'E:\DelawareBasin\DelawareBasin_geodata\workingdata\QCoutputs_v12_2.gdb\step07_EXBYMASK_{}_{}_v12_2'.format(
                unitname, extension))
...
def extract_by_mask(inRasters, maskdata, unitnames):
    ...
    for raster, mask, unitname in zip(inRasters, inMaskData, unitnames[1:]):
        ...
        outExtractByMask = ExtractByMask(raster, mask)
        outExtractByMask.save(
            r'E:\DelawareBasin\DelawareBasin_geodata\workingdata\QCoutputs_v12_2.gdb\step06_EXBYMASK_{}_discext_v12_2'.format(
                unitname))
...
def minus(rasters, unitnames):
    ...
    # ## do shelf-margin process with lochoan_base and cap_base ##
    # outMinus = Minus(lochoan_base_sm, cap_base)
    # name = 'step04_MINUS_R07lochoan_minus_cap'
    # outMinus.save(name)
    # output_rasters.append(name)
    # print('...calculated thickness of', cap_base, 'from lower ochoan')
...
def mosaic_min_ls(rasters, unitnames):
    ...
    output_location = make_gdb_name('QCoutputs_v12_2')
...
def main():
    ...
    #
    # # mos1 = mosaic_ctc(db_for_mosaic_surfaces, ps_surfs)
    #
    ...
    #
    #
    # # lochoan_base_sm = fe_ras[7]
    # # cap_base = fe_ras[9]
    # # print(lochoan_base_sm, cap_base)
    #
    # thickness_ras = minus(fe_ras, db_surfaces)
    #
    # maskdata = reclassify(thickness_ras, db_surfaces)
    # # fe_ras.append(lochoan_base_sm)
    #
    # disc_exts = extract_by_mask(fe_ras, maskdata, db_surfaces)
    #
    #
    #
    # # rasters_for_masking = ['step06_EXBYMASK_step0_MOSAIC_MIN02_LS_step0_MOSAIC_MIN01_UNITS_R01_alvbase_step01_resample_fullextent_fullextent_discext_v5',
    # #                        'step06_EXBYMASK_MOSAIC_MIN02_LS_MOSAIC_MIN01_UNITS_R04_ldockum_base_resample_fullextent_fullextent_discext_v5',
    # #                        'step06_EXBYMASK_MOSAIC_MIN02_LS_MOSAIC_MIN01_UNITS_R05_deweylake_base_resample_fullextent_fullextent_discext_v5',
    # #                        'step06_EXBYMASK_MOSAIC_MIN02_LS_MOSAIC_MIN01_UNITS_R06_uochoan_base_resample_fullextent_fullextent_discext_v5']
    # #
    # # geomask = 'geooutcropclipmask2'
    #
    # # mask_ras_with_geo_outcrop(rasters_for_masking, geomask)
    # # ebm_modelbound(disc_exts, 'discext')
    #
